...
public final class AWTGUI 
{
	...
	private void setActionListeners() 
	{
		...
		{
			midiPlayer[i].addItemListener(new ItemListener() 
			{
				public void itemStateChanged(ItemEvent e) 
				{
					if(midiPlayer[i].getState())
					{ 
						config.updatePlayerNum(""+numPlayers[i]); 
						hasPendingChange = true;

						// Uncheck all other checkboxes for midi players
						for(int j = 0; j < midiPlayers.length; j++)
						{
							if(j == i) { continue; }
							midiPlayers[j].setState(false);
						}
	...
	public void updateOptions() 
	{
			...
			//use2DHWAccel.setState(config.settings.get("2DHWAcceleration").equals("on"));
