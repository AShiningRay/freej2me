...
public final class AWTGUI 
{
	...
	final Menu midiPlayerNum = new Menu("Max MIDI Players");
	...
	final CheckboxMenuItem use2DHWAccel = new CheckboxMenuItem("HW-Accelerated Java2D", false);
	final CheckboxMenuItem useSmoothing = new CheckboxMenuItem("Use Smoothed Rendering", false);
	...
	final CheckboxMenuItem midiPlayers[] = new CheckboxMenuItem[] {new CheckboxMenuItem("1 Slot", false), new CheckboxMenuItem("2 Slots", false), 
	...
	private void setActionListeners() 
	{
		...
		use2DHWAccel.addItemListener(new ItemListener() 
		{
			public void itemStateChanged(ItemEvent e) 
			{
				if(use2DHWAccel.getState()){ config.update2DHWAccel("on"); hasPendingChange = true; }
				else{ config.update2DHWAccel("off"); hasPendingChange = true;  }
		...
		useSmoothing.addItemListener(new ItemListener() 
		{
			public void itemStateChanged(ItemEvent e) 
			{
				if(useSmoothing.getState()){ config.update2DRenderHint("Quality"); hasPendingChange = true; }
				else{ config.update2DRenderHint("Fast"); hasPendingChange = true;  }
			}
		});

		midiPlayers[0].addItemListener(new ItemListener() 
		{
			public void itemStateChanged(ItemEvent e) 
			{
				if(midiPlayers[0].getState())
				{
					config.updateMIDIPlayers(""+numPlayers[0]); 
					hasPendingChange = true;

					// Uncheck all other checkboxes for midi players
					for(int i = 0; i < midiPlayers.length; i++)
					{
						if(midiPlayers[i].equals(midiPlayers[0])) { continue; }
						midiPlayers[i].setState(false);
					}
		...
		midiPlayers[1].addItemListener(new ItemListener() 
		{
			public void itemStateChanged(ItemEvent e) 
			{
				if(midiPlayers[1].getState())
				{
					config.updateMIDIPlayers(""+numPlayers[1]); 
					hasPendingChange = true;

					// Uncheck all other checkboxes for midi players
					for(int i = 0; i < midiPlayers.length; i++)
					{
						if(midiPlayers[i].equals(midiPlayers[1])) { continue; }
						midiPlayers[i].setState(false);
					}
		...
		midiPlayers[2].addItemListener(new ItemListener() 
		{
			public void itemStateChanged(ItemEvent e) 
			{
				if(midiPlayers[2].getState())
				{
					config.updateMIDIPlayers(""+numPlayers[2]); 
					hasPendingChange = true;

					// Uncheck all other checkboxes for midi players
					for(int i = 0; i < midiPlayers.length; i++)
					{
						if(midiPlayers[i].equals(midiPlayers[2])) { continue; }
						midiPlayers[i].setState(false);
					}
		...
		midiPlayers[3].addItemListener(new ItemListener() 
		{
			public void itemStateChanged(ItemEvent e) 
			{
				if(midiPlayers[3].getState())
				{
					config.updateMIDIPlayers(""+numPlayers[3]); 
					hasPendingChange = true;

					// Uncheck all other checkboxes for midi players
					for(int i = 0; i < midiPlayers.length; i++)
					{
						if(midiPlayers[i].equals(midiPlayers[3])) { continue; }
						midiPlayers[i].setState(false);
					}
		...
		midiPlayers[4].addItemListener(new ItemListener() 
		{
			public void itemStateChanged(ItemEvent e) 
			{
				if(midiPlayers[4].getState())
				{
					config.updateMIDIPlayers(""+numPlayers[4]); 
					hasPendingChange = true;

					// Uncheck all other checkboxes for midi players
					for(int i = 0; i < midiPlayers.length; i++)
					{
						if(midiPlayers[i].equals(midiPlayers[4])) { continue; }
						midiPlayers[i].setState(false);
					}
		...
		midiPlayers[5].addItemListener(new ItemListener() 
		{
			public void itemStateChanged(ItemEvent e) 
			{
				if(midiPlayers[5].getState())
				{
					config.updateMIDIPlayers(""+numPlayers[5]); 
					hasPendingChange = true;

					// Uncheck all other checkboxes for midi players
					for(int i = 0; i < midiPlayers.length; i++)
					{
						if(midiPlayers[i].equals(midiPlayers[5])) { continue; }
						midiPlayers[i].setState(false);
					}
		...
		midiPlayers[6].addItemListener(new ItemListener() 
		{
			public void itemStateChanged(ItemEvent e) 
			{
				if(midiPlayers[6].getState())
				{
					config.updateMIDIPlayers(""+numPlayers[6]); 
					hasPendingChange = true;

					// Uncheck all other checkboxes for midi players
					for(int i = 0; i < midiPlayers.length; i++)
					{
						if(midiPlayers[i].equals(midiPlayers[6])) { continue; }
						midiPlayers[i].setState(false);
					}
		...
		midiPlayers[7].addItemListener(new ItemListener() 
		{
			public void itemStateChanged(ItemEvent e) 
			{
				if(midiPlayers[7].getState())
				{
					config.updateMIDIPlayers(""+numPlayers[7]); 
					hasPendingChange = true;

					// Uncheck all other checkboxes for midi players
					for(int i = 0; i < midiPlayers.length; i++)
					{
						if(midiPlayers[i].equals(midiPlayers[7])) { continue; }
						midiPlayers[i].setState(false);
					}
		...
		midiPlayers[8].addItemListener(new ItemListener() 
		{
			public void itemStateChanged(ItemEvent e) 
			{
				if(midiPlayers[8].getState())
				{
					config.updateMIDIPlayers(""+numPlayers[8]); 
					hasPendingChange = true;

					// Uncheck all other checkboxes for midi players
					for(int i = 0; i < midiPlayers.length; i++)
					{
						if(midiPlayers[i].equals(midiPlayers[8])) { continue; }
						midiPlayers[i].setState(false);
					}
	...
	private void buildMenuBar() 
	{
		...
		optionMenu.add(use2DHWAccel);
		optionMenu.add(useSmoothing);
		...
		for(int i = 0; i < midiPlayers.length; i++) { midiPlayerNum.add(midiPlayers[i]); }
	...
	public void updateOptions() 
	{
			...
			use2DHWAccel.setState(config.sysSettings.get("2DHWAcceleration").equals("on"));
			useSmoothing.setState(config.sysSettings.get("2DRenderHint").equals("Quality"));
			...
			for(int i = 0; i < midiPlayers.length; i++) 
			{
				midiPlayers[i].setState(config.settings.get("maxmidiplayers").equals(""+numPlayers[i]));
			}
